...
#define KNOWN_PORTS (48+6)

static HANDLE port_handles[KNOWN_PORTS] = {0};
static char port_names[KNOWN_PORTS][16] = {"/dev","/dev/","/dev","/dev","/dev","/dev",
...
bool ports_found_and_assigned = false;

void assign_known_ports(){
    //FILE *fp = popen("ls /dev/serial/by-id/* | grep FLIR_Boson", "r");
    //FILE *fp = popen("ls -l /dev/serial/by-id/* | grep FLIR_Boson | grep -o \"ttyACM[0-9]\"","r");
    FILE *fp = popen("ls -l /dev/serial/by-id/* | grep FLIR_Boson | grep -o -E 'tty[A-Za-z]?[A-Za-z]?[A-Za-z]?[0-9]?'", "r");
    char pname[20];
    for(int i = 0; i < 6; i++){
        if(feof(fp)) break;
        sprintf(pname,"");
        fscanf(fp, "%s\n", pname); // \n so that once it finished reading it will go to the next line immediately, thus will hit feof after reading the last meaningful line
        if(strlen(pname) > 0){
            sprintf(port_names[i], "/dev/%s", pname);
            ports_found_and_assigned = true;
        }
        printf("port name = %s, port_names[%d] = %s\n", pname, i, port_names[i]);
    }
    fclose(fp);
}

int32_t FSLP_lookup_port_id( char *port_name, int32_t len ){
    if(!ports_found_and_assigned){
        assign_known_ports();
    }
    int32_t port_id = -1;
    if (len > 15)
