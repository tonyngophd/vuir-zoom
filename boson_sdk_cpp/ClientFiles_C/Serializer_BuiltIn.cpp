...
void byteToBOOL(const uint8_t *inBuff, bool *outVal){
	*outVal = (bool) *inBuff;
}
void byteToBOOLArray(const uint8_t *inBuff, bool *outVal,uint16_t length){
	uint16_t i;
	for (i=0;i<length;i++) {
...
void BOOLToByte(const bool inVal, const uint8_t *outBuff){
	uint8_t *outPtr = (uint8_t *)outBuff;
	*outPtr = *((uint8_t *) (&inVal));
}
void BOOLArrayToByte(const bool *inVal,uint16_t length, const uint8_t *outBuff){
	uint16_t i;
	for (i=0;i<length;i++){
