...
#include <libgwavi/src/avi-utils.h>
}
...
{
public:
    QAviWriter(const QString& = "", const QSize& = QSize(640, 512), unsigned int = 25, const QString& = "MJPG");
	~QAviWriter();

	bool open();
    bool open(QString outputFileName);
    bool open(QString outputFileName, QSize size);
	bool close();
    bool isOpened();
    ...
    bool changeVideoSize(const QSize&);
