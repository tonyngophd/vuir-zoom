...
QAviWriter::QAviWriter(const QString& fileName, const QSize& size, unsigned int fps, const QString& codec)
    : QObject()
...
bool QAviWriter::open()
{
    gwavi = gwavi_open(d_file_name.toUtf8().constData(), (unsigned int)d_size.width(), (unsigned int)d_size.height(),
            d_codec.toLatin1().constData(), d_fps, ((!d_audio_file_name.isEmpty() && parseAudioFileFormat()) ? &d_audio_format : NULL));
    _isOpened = (gwavi != nullptr);
    return _isOpened;
//    return (gwavi != nullptr);
}

bool QAviWriter::open(QString outputFileName){
    if(outputFileName != ""){
        d_file_name = outputFileName;
        return this->open();
    }
    return false;
}

bool QAviWriter::open(QString outputFileName, QSize size){
    if(outputFileName != "" && size != QSize(0,0)){
        d_file_name = outputFileName;
        d_size = size;
        return this->open();
    }
...
void QAviWriter::setOutputFileName(const QString& fileName){
    d_file_name = fileName;
}
 ...
 */
bool QAviWriter::changeVideoSize(const QSize& size)
{
    if (d_size == size)
        return false;

    int error = gwavi_set_size(gwavi, (unsigned int)size.width(), (unsigned int)size.height());
    qDebug() << "error from gwavi_set_size" << error;
    if (!error)
        d_size = size;
